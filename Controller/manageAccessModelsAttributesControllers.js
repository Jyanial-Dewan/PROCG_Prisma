...
exports.getManageAccessModelAttributes = async (req, res) => {
  try {
    const result = await prisma.manage_access_model_attributes.findMany();
...
exports.getUniqueManageAccessModelAttribute = async (req, res) => {
  try {
    ...
    const result = await prisma.manage_access_model_attributes.findUnique({
      where: {
        id: Number(id),
      },
    });
...
exports.createManageAccessModelAttribute = async (req, res) => {
  try {
    const response = await prisma.manage_access_model_attributes.findMany();
    const id = Math.max(...response.map((item) => item.id));
    const data = req.body;
    const result = await prisma.manage_access_model_attributes.create({
      data: {
...
exports.updateManageAccessModelAttribute = async (req, res) => {
  try {
    ...
    const findExistId = await prisma.manage_access_model_attributes.findFirst({
      where: {
        id: id,
      },
    });
    ...
    // Validation  End/---------------------------------/
    const result = await prisma.manage_access_model_attributes.update({
      where: {
        id: id,
      },
      data: {
...
exports.upsertManageAccessModelAttribute = async (req, res) => {
  ...
  const response = await prisma.manage_access_model_attributes.findMany();
  const id = Math.max(...response.map((item) => item.manage_access_model_id));
  const upsertResults = [];

  try {
    for (const item of data) {
      const result = await prisma.manage_access_model_attributes.upsert({
        where: { id: item.id },
        update: {
...
exports.deleteManageAccessModelAttribute = async (req, res) => {
  try {
    ...
    const findManageAccessModelAttributeId =
      await prisma.manage_access_model_attributes.findUnique({
        where: {
          id: id,
        },
      });
    ...
    // Validation  End/---------------------------------/
    const result = await prisma.manage_access_model_attributes.delete({
      where: {
        id: id,
      },
    });
